...
//RWlock codes
int my_rwlock_init(my_rwlock_t* rwlock_p) {
    if (pthread_mutex_init(&rwlock_p->mutex, NULL) != 0) return -1;
    if (pthread_cond_init(&rwlock_p->readers, NULL) != 0) return -1;
    if (pthread_cond_init(&rwlock_p->writers, NULL) != 0) return -1;
    rwlock_p->readers_count = 0;
    rwlock_p->writers_count = 0;
    rwlock_p->writer_waiting = 0;
    return 0;
}

int my_rdlock(my_rwlock_t* rwlock_p) {
    pthread_mutex_lock(&rwlock_p->mutex);
    while (rwlock_p->writers_count > 0 || rwlock_p->writer_waiting > 0) {
        pthread_cond_wait(&rwlock_p->readers, &rwlock_p->mutex);
    }
    rwlock_p->readers_count++;
    pthread_mutex_unlock(&rwlock_p->mutex);
    return 0;
}

int my_wrlock(my_rwlock_t* rwlock_p) {
    pthread_mutex_lock(&rwlock_p->mutex);
    rwlock_p->writer_waiting++;
    while (rwlock_p->writers_count > 0 || rwlock_p->readers_count > 0) {
        pthread_cond_wait(&rwlock_p->writers, &rwlock_p->mutex);
    }
    rwlock_p->writer_waiting--;
...
int my_unlock(my_rwlock_t* rwlock_p) {
    ...
    if (rwlock_p->writers_count > 0) {
        rwlock_p->writers_count--;
    } else if (rwlock_p->readers_count > 0) {
        rwlock_p->readers_count--;
    }
    if (rwlock_p->writers_count == 0 && rwlock_p->writer_waiting > 0) {
        pthread_cond_signal(&rwlock_p->writers);
    } else if (rwlock_p->writers_count == 0 && rwlock_p->writer_waiting == 0) {
        pthread_cond_broadcast(&rwlock_p->readers);
    }
    pthread_mutex_unlock(&rwlock_p->mutex);
    return 0;
}
...
int my_rwlock_destroy(my_rwlock_t* rwlock_p) {
    pthread_mutex_destroy(&rwlock_p->mutex);
    pthread_cond_destroy(&rwlock_p->readers);
    pthread_cond_destroy(&rwlock_p->writers);
    return 0;
} 
